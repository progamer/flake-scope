...
test('renames the profile to Alice A', async ({ page }) => {
  test.info().annotations.push({ type: 'flakescope:resource', description: 'account:alice' });

  await page.goto('/profile');
  await page.getByLabel('Display name').fill('Alice A');
  await page.getByRole('button', { name: 'Save' }).click();
  await expect(page.getByTestId('display-name')).toHaveText('Alice A');
